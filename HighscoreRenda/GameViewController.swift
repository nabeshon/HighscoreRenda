...
class GameViewController: UIViewController {
    ...
    //ハイスコアを更新したことを表示するLabelの変数を準備する
...
//            getHighscore()
    ...
    @objc func up() {
        ...
        if remain <= 0.0 && cooltime == 3.0 {
            ...
            if personalHighscore < tapCount {
                ...
                //Firestoreのデータを読み込む
                firestore.collection("highscore").document(String(Int(maxremain))+"s").getDocument {snapshot, error in
                    if error != nil {
                        print("エラーが発生しました")
                        print(error)
                        return
                    ...
                    if(self.globalHighscore < self.tapCount){
                        ...
                        self.newHighscoreLabel.text = "世界ハイスコア更新！"
                        
                    } else {
                        self.newHighscoreLabel.text = "自己ハイスコア更新！"
                    }
    ...
    override func viewDidLoad() {
        ...
        rankingButton.layer.cornerRadius = 15
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        rankingViewController.maxremain = self.maxremain
    }
